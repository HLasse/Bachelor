...
contDepPredictionsProb$ensembleMax <- apply(contDepPredictionsProb, 1, max)
#pooling predictions with mean
contDepPredictionsProb$ensembleMean <- apply(contDepPredictionsProb, 1, mean)
#pooling preditions with median
contDepPredictionsProb$ensembleMedian <- apply(contDepPredictionsProb, 1, median)
...
#try different types of ensembles
#choosing the most confident predictor
litPredictionsProb$ensembleMax <- apply(litPredictionsProb, 1, max)
#pooling predictions with mean
litPredictionsProb$ensembleMean <- apply(litPredictionsProb, 1, mean)
#pooling preditions with median
litPredictionsProb$ensembleMedian <- apply(litPredictionsProb, 1, median)
...
#distribution of error checking
```{r}
...
#try different types of ensembles
#choosing the most confident predictor
v2PredictionsProb$ensembleMax <- apply(v2PredictionsProb, 1, max)
#pooling predictions with mean
v2PredictionsProb$ensembleMean <- apply(v2PredictionsProb, 1, mean)
...
v2Error <- v2PredictionsProb %>%
  mutate(id = depV2$id, diagnosis = depV2$DiagnosisInteger) %>%
  group_by(id) %>%
  summarise_if(is.numeric, mean) %>%
  arrange.vars(., c('diagnosis' = 2))
#  mutate_if(is.numeric, ifelse(. > 0.5, 'Depression', 'Control'))

#adding column with mean prediction
v2Error$meanPred <- apply(v2Error[,3:6], 1, mean)
v2Error[,3:9] <- round(v2Error[,3:9], 2)
arrange.vars(v2Error, c('meanPred' = 7))
```
...
#try different types of ensembles
#choosing the most confident predictor
depPredictionsProb$ensembleMax <- apply(depPredictionsProb, 1, max)
#pooling predictions with mean
...
aucDf <- arrange.vars(aucDf, "model" = 1)
