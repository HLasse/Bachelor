...
p_load(stringr, beepr, groupdata2, glmnet, brms, caret, kernlab, klaR, glmnet, tidyverse, knitr, pROC, randomForest, xgboost)
```
...
# XX 30 SECONDS WINDOWS WITH DEPRESSION IN TEST SET - 63 30 SECONDS WINDOWS WITH NO DEPRESSION
# XX 30 SECOND WINDOWS WITH DEPRESSION IN TRAINING SET - 305 30 SECONDS WINDOWS WITH NO DEPRESSION
...
depFormula <- dep %>%
  select(depPredictors, Remission, id)

#soooo many predictors
depLogistic <- brm(Remission ~ MCEP_22_IQR +HMPDM_20_median + MCEP_11_median + MDQ_median + HMPDM_18_median + HMPDD_3_median + 
MCEP_4_median + MCEP_11_IQR + HMPDM_21_IQR + HMPDM_16_median + HMPDM_14_median + MCEP_13_median + 
HMPDM_20_IQR + VUV_median + sdPauseLength + HMPDD_9_IQR + H1H2_IQR + MCEP_8_IQR + HMPDM_10_IQR +
HMPDM_23_median + MCEP_1_IQR + HMPDD_7_IQR + HMPDM_12_median + HMPDM_22_IQR + HMPDM_19_IQR + MCEP_1_median +
MCEP_10_median + MCEP_2_IQR + f0_IQR + HMPDM_19_median + HMPDM_21_median + MCEP_17_IQR + HMPDM_13_median + 
MCEP_20_median + medianPauseLength + F4_median + MCEP_7_IQR + HMPDM_17_median + HRF_median + Rd_conf_median + 
MCEP_2_median + HMPDM_22_median + HMPDD_6_IQR + peakSlope_median + MCEP_0_IQR + MCEP_17_median + HMPDD_4_IQR +
MCEP_21_IQR + MCEP_3_median + MCEP_8_median + HMPDD_7_median  + F3_median + MCEP_5_median + MCEP_13_IQR + 
MCEP_4_IQR +  HMPDD_6_median + MCEP_9_median +  HMPDD_5_median + MCEP_15_median + MCEP_0_median + MCEP_7_median +     MCEP_19_median  +  MCEP_12_median +  (1|id), 
            data = dep, family = bernoulli(), cores = 4, chains = 4, control = list( adapt_delta = .99, max_treedepth = 12))


depLogistic2 <- brm(Remission ~.-id + (1|id), 
...
predict(depLogistic, newdata = depTest)
...
controls for caret's train function
```{r}
...
svmLinNoWeights <- train(x = dep[,depPredictors], y = dep$Remission,
                    method = "svmLinear",
                    metric = "ROC",
                    trControl=ctrl,
                    tuneGrid = grid,
...
svmLinWeighted <- train(x = dep[,depPredictors], y = dep$Remission,
                    method = "svmLinear",
                    metric = "Kappa",
                    trControl=ctrlNoProb,
                    tuneGrid = grid,
                    tuneLength = 10, 
                    class.weights = c(NoRemission = 1, Remission = 15))
...
#### no class weights
svmRbfNoWeights <- train(dep[,depPredictors], dep$Remission, 
                   method = "svmRadial", 
                   metric = "ROC", 
                   tuneGrid = svmRGrid2, 
                   trControl = ctrl)
...
#### with class weights
svmRbfWeighted <- train(dep[,depPredictors], dep$Remission, 
                   method = "svmRadial", 
                   metric = "Kappa", 
                   tuneGrid = svmRGrid2, 
                   trControl = ctrlNoProb,
...
#### ^^ IS THIS CHEATING?  TRAIN THRESHOLD ON TRAIN DATA INSTEAD???
```


```{r  svm predicting diagnosis}
############################# LINEAR SVM ¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤
load('contDepNet.RData')

contPreds <- contDepNet %>% 
  filter(abs(betas) > 0.1)
contDepPredictors <- contPreds$predictors
...
ctrl <- trainControl(method = "LGOCV", #1leave group out cv
                     summaryFunction = twoClassSummary, #use AUC to pick best model
                     classProbs = T)                    #output probabilities not only classification
...
svm_Linear <- train(x = contDep[,contDepPredictors], y = contDep$Diagnosis,
                    method = "svmLinear",
                    metric = "ROC",
                    trControl=ctrl,
                    tuneGrid = grid,
                    tuneLength = 10, 
                    class.weights = c(Control = 1, Depression = 15))

svm_Linear
############################# RBF SVM ¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤

#finding optimal signma and cost parameters
sigmaRange <- sigest(as.matrix(contDep[,contDepPredictors]))
svmRGrid <- expand.grid(.sigma = sigmaRange[1], .C = 2^(seq(-4, 4)))


svmRModel <- train(contDep[,contDepPredictors], contDep$Diagnosis, 
                   method = "svmRadial", 
                   metric = "ROC", 
                   tuneGrid = svmRGrid, 
                   trControl = ctrl, 
...
#### LINEAR
contDepTest$Diagnosis <- ifelse(depTest$Remission == 0, 'NoRemission', 'Remission')

contDepTest$PredictionsLinear <- predict(svm_Linear, newdata = contDepTest[,contDepPredictors])

confusionMatrix(data = contDepTest$PredictionsLinear, reference = contDepTest$Diagnosis, positive = "Depression")


#### RBF
contDepTest$PredictionsRbf <- predict(svmRModel, newdata = contDepTest[,contDepPredictors])

confusionMatrix(data = contDepTest$PredictionsRbf, reference = contDepTest$Diagnosis, positive = "Depression")


########## CHANGE VARIABLES TO THE CORRECT ONES ##################################
...
#Random forest
```{r}
rf <- train(x = dep[,depPredictors], y = dep$Remission,
                    method = "rf",
                    metric = "ROC",
                    trControl=ctrl)

depTest$rf <- predict(rf, newdata = depTest[,depPredictors])

confusionMatrix(data = depTest$rf, reference = depTest$Remission, positive = "Remission")
#everything is remission
##try up/downsampling?
...
nbRemissionGrid1 <- train(x = dep[,depPredictors],y = dep$Remission,
              method = 'nb',
              trControl = ctrl,
...
nbRemissionGrid2 <- train(x = dep[,depPredictors],y = dep$Remission,
              method = 'nb',
              trControl = ctrl,
              metric = "ROC", 
              tuneGrid = grid2,
...
plot(nbRemissionGrid1)

nbRemissionGrid1$results %>% 
  top_n(5, wt = ROC) %>%
  arrange(desc(ROC))
...
### SETTING PRIORS? doesn't seem to do much..
```

K-Nearest neighbors
```{r}

knnFit <- train(dep[,depPredictors], dep$Remission, 
                method = "knn", 
                metric = "ROC", 
...
#adding predictions
#svm models
depResults$svmLinNoWeights <- predict(svmLinNoWeights, newdata = depTest[,depPredictors])
depResults$svmLinNoWeightsProb <- predict(svmLinNoWeights, newdata = depTest[,depPredictors], type = 'prob')$Remission
depResults$svmLinWeighted <- predict(svmLinWeighted, newdata = depTest[,depPredictors])

depResults$svmRbfNoWeights <- predict(svmRbfNoWeights, newdata = depTest[,depPredictors])
depResults$svmRbfNoWeightsProb <- predict(svmRbfNoWeights, newdata = depTest[,depPredictors], type = 'prob')$Remission
depResults$svmRbfWeighted <- predict(svmRbfWeighted, newdata = depTest[,depPredictors])
#random forest
depResults$rf <- predict(rf, newdata = depTest[,depPredictors])
depResults$rfProp <- predict(rf, newdata = depTest[,depPredictors], type = 'prob')$Remission
#xgboost models
depResults$xgbBase <- predict(xgbBase, newdata = depTest[,depPredictors])
depTest$xgbBaseProp <- predict(xgbBase, newdata = depTest[,depPredictors], type = 'prob')$Remission
depResults$xgbFit <- predict(xgbFit, newdata = depTest[,depPredictors])
depTest$xgbFitProp <- predict(xgbFit, newdata = depTest[,depPredictors], type = 'prob')$Remission
#naive bayes
depResults$NaiveBayes <- predict(nbRemissionGrid1, newdata = depTest[,depPredictors])  
depTest$NaiveBayesProp <- predict(nbRemissionGrid1, newdata = depTest[,depPredictors], type = 'prob')$Remission
#knn
depResults$knn <- predict(knnFit, newdata = depTest[,depPredictors])
depTest$knnProp <- predict(knnFit, newdata = depTest[,depPredictors], type = 'prob')$Remission
...
#confusion matrices
confusionMatrix(data = depTest$knn, reference = depTest$Remission, positive = "Remission")
confusionMatrix(data = depTest$NaiveBayes, reference = depTest$Remission, positive = "Remission")
confusionMatrix(data = depTest$xgbFit, reference = depTest$Remission, positive = "Remission")
confusionMatrix(data = depTest$PredictionsLinear, reference = depTest$Remission, positive = "Remission")
confusionMatrix(data = depTest$xgbBase, reference = depTest$Remission, positive = "Remission")
confusionMatrix(data = depTest$PredictionsRbf, reference = depTest$Remission, positive = "Remission")
...
rfROC <- roc(depTest$Remission, depTest$rfProp, levels = rev(levels(depTest$Remission)))
rfROC

xgbBaseROC <- roc(depTest$Remission, depTest$xgbBaseProp, levels = rev(levels(depTest$Remission)))
xgbBaseROC

xgbFitROC <- roc(depTest$Remission, depTest$xgbFitProp, levels = rev(levels(depTest$Remission)))
xgbFitROC

NaiveBayesROC <- roc(depTest$Remission, depTest$NaiveBayesProp, levels = rev(levels(depTest$Remission)))
NaiveBayesROC

knnROC <- roc(depTest$Remission, depTest$knnProp, levels = rev(levels(depTest$Remission)))
knnROC
...
plot(SVMLinNoWeightsROC, legacy.axes = TRUE, col = 'black')
plot(SVMRbfNoWeightsROC, legacy.axes = TRUE, add = T, col = 'red')
plot(rfROC, legacy.axes = TRUE, col = 'black')
plot(xgbFitROC, legacy.axes = TRUE, col = 'black')
plot(xgbBaseROC, legacy.axes = TRUE, col = 'black')
plot(NaiveBayesROC, legacy.axes = TRUE, col = 'black')
plot(knnROC, legacy.axes = TRUE, col = 'black')
